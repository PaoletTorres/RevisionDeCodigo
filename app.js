...
async function displayUser(username) { // se agrega async ya que se esta usando await 
  try{ // se agrega un bloque try-catch para poder capturar el error que se lanza con throw new Error
   ...
   if(!response.ok) throw new Error (`Usario no encontrado`); // se agrega esta instrucción es para lanzar un error de forma intencional 
      const data = await 
...
function handleError(err) {
  console.log('OH NO!');
  console.log(err);
  n.textContent = `Algo salió mal: ${err.message}`; // faltaba agregar .message 
}
